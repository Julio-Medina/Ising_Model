...
# -*- coding: utf-8 -*-
"""
Created on Sat May 27 21:34:09 2023
...
class isingModel(object): # Ising Model object
    ...
    def __init__(self,N,iterations,minTemp, maxTemp,JKb):
        self.N=N#Dimension del reticulo(red) 
        self.iterations=iterations#Numero de iteraciones
        self.lattice=[]#matriz N*N
        self.initialLattice=[]#inital lattice red inicial
    ...
    def nearestNeighborEnergy(self,x,y):#calcula la energia de los vecinos mas cercanos
        x1=x2=x3=x4=x
        y1=y2=y3=y4=y
    ...
    def using_shifts(self):
        N=self.N
        shifter = numpy.arange(N*N).reshape( (N, N) )
        print(shifter)
    ...
    def runMonteCarlo(self,T):
        for i in range(self.iterations):
            ...
            energy=2*self.nearestNeighborEnergy(randomX,randomY)
            if energy>0:
...
test=isingModel(100,15000,1,20,5);
#print(test.lattice)
